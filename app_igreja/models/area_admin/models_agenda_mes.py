"""Agenda do mês: master (mês) e itens por dia (admin)."""
from django.db import models

from .models_modelo import TBMODELO


class TBAGENDAMES(models.Model):
    """Master: um registro por mês (primeiro dia do mês)."""

    AGE_ID = models.AutoField(primary_key=True, verbose_name="ID")
    AGE_MES = models.DateField(verbose_name="Mês", unique=True, help_text="Primeiro dia do mês")
...
class TBITEAGENDAMES(models.Model):
    """Detail: um item por dia do mês (modelo, horário, encargos)."""
    ...
    AGE_ITE_MODELO = models.IntegerField(verbose_name="Modelo", null=True, blank=True, help_text="ID do modelo")
    AGE_ITE_HORARIO = models.TimeField(verbose_name="Horário", null=True, blank=True)
    AGE_ITE_ENCARGOS = models.TextField(verbose_name="Encargos", blank=True, null=True)
    ...
    def __str__(self):
        ...
        if self.AGE_ITE_MODELO:
            try:
                modelo_nome = TBMODELO.objects.get(pk=self.AGE_ITE_MODELO).MOD_DESCRICAO
